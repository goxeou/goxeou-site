...
class KanjiaOrder extends Common
{
	...
	public function excel(){
		...
		foreach($list as $k=>$vo){
            ...
            //配送自定义表单
            $vo['formdata'] = \app\models\Freight::getformdata($vo['id'],'kanjia_order');
            if($vo['formdata']) {
	...
	public function getdetail(){
		...
		$order['formdata'] = \app\models\Freight::getformdata($order['id'],'kanjia_order');
	...
	public function sendExpress(){
        ...
        if($order['platform'] == 'wx' && $order['paytypeid'] == 2){
            \app\commons\Order::wxShipping(aid,$order,'kanjia',['express_com'=>$express_com,'express_no'=>$express_no]);
        }
	...
	public function plfh(){
		...
		\app\commons\System::plog('砍价活动订单批量发货');
    ...
    function orderHexiao(){
        ...
        //发货信息录入 微信小程序+微信支付
        if($order['platform'] == 'wx' && $order['paytypeid'] == 2){
            \app\commons\Order::wxShipping(aid,$order,'kanjia');
        }
	...
	public function refundCheck(){
		...
		if($st==2){
            ...
            //退款减去商户销量
            \app\models\Payorder::addSales($orderid,'kanjia',$order['aid'],$order['bid'],-$order['num']);
			//积分抵扣的返还
			if($order['scoredkscore'] > 0){
				\app\commons\Member::addscore(aid,$order['mid'],$order['scoredkscore'],'订单退款返还');
			}
			//扣除消费赠送积分
        	\app\commons\Member::decscorein(aid,'kanjia',$order['id'],$order['ordernum'],'订单退款扣除消费赠送');
			//退款退还佣金
			//退款成功通知
			$tmplcontent = [];
