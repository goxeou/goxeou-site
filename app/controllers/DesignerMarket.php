...
class DesignerMarket extends Common
{
	...
	public function index(){
		if(request()->isAjax()){
			...
			foreach($data as $k=>$v){
                $data[$k]['type_txt'] = \app\commons\Coupon::getCouponTypeTxt($v['type']);
	...
	public function edit(){
		...
		if(input('param.id')){
			...
			$info = array('id'=>'','perlimit'=>1,'yxqtype'=>1,'yxqtime'=>date('Y-m-d 00:00:00').' ~ '.date('Y-m-d 00:00:00',time()+7*86400),'starttime'=>date('Y-m-d 00:00:00'),'endtime'=>date('Y-m-d 00:00:00',time()+7*86400),'gettj'=>'-1','showtj'=>'-1','usetj'=>'-1','sort'=>0,'fwtype'=>0);
		}
	...
	public function creategiveqr(){
		...
		$dir = 'upload/'.date('Ym').'/'.date('d_His').rand(1000,9999);
		if(!is_dir(ROOT_PATH.$dir)) mk_dir(ROOT_PATH.$dir);
		$zippath = ROOT_PATH.$dir.'.zip';

		foreach($list as $record){
			$page = 'pages/coupon/coupondetail?id='.$record['couponid'].'&pid='.$record['mid'].'&rid='.$record['id'];
			...
			$url = 'https://api.weixin.qq.com/wxa/getwxacodeunlimit?access_token='.\app\commons\Wechat::access_token(aid,'wx');
			$data = array();
			...
			if($errmsg){
				return json(['status'=>0,'msg'=>\app\commons\Wechat::geterror($errmsg),'rs'=>$errmsg,'data'=>$data]);
			}
	...
	public function creategiveqr2(){
		...
		$page = 'pages/coupon/coupondetail?id='.$record['couponid'].'&pid='.$record['mid'].'&rid=all'.$record['id'];
		$rs = \app\commons\Wechat::getQRCode(aid,'wx',$page);
		$rs['page'] = $page;
		return json($rs);
	}
