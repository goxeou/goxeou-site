...
class YuyueWorkerCategory extends Common
{
	...
	public function save(){
		...
		if($info['id']){
			...
			\app\commons\System::plog('编辑'.$set['diyname']?$set['diyname']:'人员'.$info['id']);
		}else{
			$info['aid'] = aid;
			$info['bid'] = bid;
			$info['createtime'] = time();
			$id = Db::name('yuyue_worker_category')->insertGetId($info);
			\app\commons\System::plog('添加'.$set['diyname']?$set['diyname']:'人员'.$id);
		}
	...
	public function setst(){
		...
		\app\commons\System::plog($set['diyname']?$set['diyname']:'人员'.'改状态'.implode(',',$ids));
	...
	public function del(){
		...
		\app\commons\System::plog($set['diyname']?$set['diyname']:'人员'.'删除'.implode(',',$ids));
