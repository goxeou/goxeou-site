...
class ApiLuckyCollage extends ApiCommon
{
	...
	public function createOrder(){
		...
		\app\models\Freight::saveformdata($orderid,'lucky_collage_order',$freight['id'],$post['formdata']);		

		$payorderid = \app\models\Payorder::createorder(aid,$orderdata['bid'],$orderdata['mid'],'lucky_collage',$orderid,$ordernum,$orderdata['title'],$orderdata['totalprice'],$pay_score);
		Db::commit(); //解锁
		//减库存加销量
		...
		\app\commons\Wechat::sendhttmpl(aid,$orderdata['bid'],'tmpl_orderconfirm',$tmplcontent,m_url('admin/order/luckycollageorder'),$orderdata['mdid'],$tempconNew);
		...
		\app\commons\Wechat::sendhtwxtmpl(aid,$orderdata['bid'],'tmpl_orderconfirm',$tmplcontent,'admin/order/luckycollageorder',$orderdata['mdid']);
	...
	public function orderdetail(){
		...
		$detail['formdata'] = \app\models\Freight::getformdata($detail['id'],'lucky_collage_order');
	...
	function orderCollect(){ //确认收货
		...
		\app\commons\Wechat::sendhttmpl(aid,$order['bid'],'tmpl_ordershouhuo',$tmplcontent,m_url('admin/order/luckycollageorder'),$order['mdid'],$tmplcontentNew);
		...
		\app\commons\Wechat::sendhtwxtmpl(aid,$order['bid'],'tmpl_ordershouhuo',$tmplcontent,'admin/order/luckycollageorder',$order['mdid']);
	...
	public function logistics(){//查快递单号
		$get = input('param.');
		//$content = ali_getwuliu($get['express_no'],$get['express']);
