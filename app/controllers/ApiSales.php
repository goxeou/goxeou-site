...
use think\facade\Db;
class ApiSales extends ApiCommon
{
// 
	function join(){
		//$this->checklogin();
	...
	function kanjiaKan(){
		...
		if($cutmoney > 0 && $product['helpgive_percent'] > 0){ //帮砍得积分或余额
			if($product['helpgive_type'] == 0){ //积分
				...
				if($givescore > 0){
					if($product['helpgive_ff'] == 0) \app\commons\Member::addscore(aid,mid,$givescore,'帮好友砍价奖励');
			...
			if($product['helpgive_type'] == 1){ //余额
				...
				if($givemoney > 0){
					if($product['helpgive_ff'] == 0) \app\commons\Member::addmoney(aid,mid,$givemoney,'帮好友砍价奖励');
	...
	function buy(){
		...
		$needLocation = 0;
		$rs = \app\models\Freight::formatFreightList($freightList,$address,$joininfo['now_price'],1,$product['weight']);
		...
		if(!$rdata['linkman']){
			$lastorder = Db::name('sales_order')->where('aid',aid)->where('mid',mid)->where('linkman','<>','')->find();
			if($lastorder){
	...
	function createOrder(){
		...
		if($post['freightid']){
			...
			$rs = \app\models\Freight::getFreightPrice($freight,$address,$product_price,1,$weight);
        ...
        if($product['bid'] > 0) {
            ...
            $totalprice_business = $product_price + $freight_price - $scoredkmoney;
            //商品独立费率
		...
		$orderid = Db::name('sales_order')->insertGetId($orderdata);
		\app\models\Freight::saveformdata($orderid,'sales_order',$freight['id'],$post['formdata']);
		...
		$payorderid = \app\models\Payorder::createorder(aid,$orderdata['bid'],$orderdata['mid'],'kanjia',$orderid,$ordernum,$orderdata['title'],$orderdata['totalprice'],$orderdata['scoredkscore']);
		...
		\app\commons\Wechat::sendhttmpl(aid,$orderdata['bid'],'tmpl_orderconfirm',$tmplcontent,m_url('admin/order/kanjiaorder'),$orderdata['mdid']);
		...
		\app\commons\Wechat::sendhtwxtmpl(aid,$orderdata['bid'],'tmpl_orderconfirm',$tmplcontent,'admin/order/kanjiaorder',$orderdata['mdid']);
	...
	function getOrderCount(){
		$this->checklogin();
		$count0 = 0 + Db::name('sales_order')->where('aid',aid)->where('mid',mid)->where('status',0)->count();
		$count1 = 0 + Db::name('sales_order')->where('aid',aid)->where('mid',mid)->where('status',1)->count();
		$count2 = 0 + Db::name('sales_order')->where('aid',aid)->where('mid',mid)->where('status',2)->count();
		$count4 = 0 + Db::name('sales_order')->where('aid',aid)->where('mid',mid)->where('status',4)->count();
	...
	function orderlist(){
		...
		$datalist = Db::name('sales_order')->where($where)->order('id desc')->page($pagenum,10)->select()->toArray();
	...
	function orderdetail(){
		$this->checklogin();
		$id = input('param.id/d');
		$detail = Db::name('sales_order')->where('id',$id)->find();	
		if(!$detail) $detail = [];
		...
		$detail['formdata'] = \app\models\Freight::getformdata($detail['id'],'sales_order');
	...
	function orderCollect(){ //确认收货
		...
		$order = Db::name('sales_order')->where('aid',aid)->where('mid',mid)->where('id',$orderid)->find();
		...
		Db::name('sales_order')->where('aid',aid)->where('mid',mid)->where('id',$orderid)->update(['status'=>3,'collect_time'=>time()]);
		...
		\app\commons\Wechat::sendhttmpl(aid,$order['bid'],'tmpl_ordershouhuo',$tmplcontent,m_url('admin/order/kanjiaorder'),$order['mdid']);
		...
		\app\commons\Wechat::sendhtwxtmpl(aid,$order['bid'],'tmpl_ordershouhuo',$tmplcontent,'admin/order/kanjiaorder',$order['mdid']);
	...
	function closeOrder(){
		...
		$post = input('post.');
		$orderid = intval($post['orderid']);
		$order = Db::name('sales_order')->where('id',$orderid)->where('aid',aid)->where('mid',mid)->find();
		if(!$order || $order['status']!=0){
			...
			$rs = Db::name('sales_order')->where('id',$orderid)->where('aid',aid)->where('mid',mid)->update(['status'=>4]);
	...
	function delOrder(){
		...
		if(!$order || $order['status']!=4){
			...
			$rs = Db::name('sales_order')->where('id',$orderid)->where('aid',aid)->where('mid',mid)->delete();
	...
	function refund(){//申请退款
		...
		if(request()->isPost()){
			...
			Db::name('sales_order')->where('aid',aid)->where('mid',mid)->where('id',$orderid)->update(['refund_time'=>time(),'refund_status'=>1,'refund_reason'=>$post['reason'],'refund_money'=>$money]);
			...
			\app\commons\Wechat::sendhttmpl(aid,$order['bid'],'tmpl_ordertui',$tmplcontent,m_url('admin/order/kanjiaorder'),$order['mdid']);
			...
			\app\commons\Wechat::sendhtwxtmpl(aid,$order['bid'],'tmpl_ordertui',$tmplcontent,'admin/order/kanjiaorder',$order['mdid']);
		...
		$order = Db::name('sales_order')->where('aid',aid)->where('mid',mid)->where('id',$rdata['orderid'])->find();
