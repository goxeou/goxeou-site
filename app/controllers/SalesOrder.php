...
class SalesOrder extends Common
{
    ...
    public function index(){
		if(request()->isAjax()){
			...
			$count = 0 + Db::name('sales_order')->where($where)->count();
			$list = Db::name('sales_order')->where($where)->page($page,$limit)->order($order)->select()->toArray();
	...
	public function excel(){
		...
		$list = Db::name('sales_order')->where($where)->order($order)->select()->toArray();
		...
		foreach($list as $k=>$vo){
            ...
            //配送自定义表单
            $vo['formdata'] = \app\models\Freight::getformdata($vo['id'],'sales_order');
            if($vo['formdata']) {
	...
	public function getdetail(){
		...
		$order = Db::name('sales_order')->where('aid',aid)->where('bid',bid)->where('id',$orderid)->find();
		
		$order['formdata'] = \app\models\Freight::getformdata($order['id'],'sales_order');
	...
	public function setremark(){
		...
		Db::name('sales_order')->where('aid',aid)->where('bid',bid)->where('id',$orderid)->update(['remark'=>$content]);
	...
	public function changeprice(){
		...
		Db::name('sales_order')->where('aid',aid)->where('bid',bid)->where('id',$orderid)->update(['totalprice'=>$newprice,'ordernum'=>date('ymdHis').aid.rand(1000,9999)]);
		return json(['status'=>1,'msg'=>'修改完成']);
	}
	//关闭订单
	public function closeOrder(){
		$orderid = input('post.orderid/d');
		$order = Db::name('sales_order')->where('id',$orderid)->where('aid',aid)->where('bid',bid)->find();
		...
		$rs = Db::name('sales_order')->where('id',$orderid)->where('aid',aid)->where('bid',bid)->update(['status'=>4]);
	...
	public function ispay(){
		...
		$orderid = input('post.orderid/d');
		Db::name('sales_order')->where('aid',aid)->where('bid',bid)->where('id',$orderid)->update(['status'=>1,'paytime'=>time(),'paytype'=>'后台支付']);
		//奖励积分
		$order = Db::name('sales_order')->where('aid',aid)->where('bid',bid)->where('id',$orderid)->find();
	...
	public function sendExpress(){
		...
		if($order['status']!=1){ //修改物流信息
			Db::name('sales_order')->where('aid',aid)->where('bid',bid)->where('id',$orderid)->update(['express_com'=>$express_com,'express_no'=>$express_no]);
			return json(['status'=>1,'msg'=>'操作成功']);
		}

		Db::name('sales_order')->where('aid',aid)->where('bid',bid)->where('id',$orderid)->update(['express_com'=>$express_com,'express_no'=>$express_no,'send_time'=>time(),'status'=>2]);
		...
		\app\commons\Wechat::sendtmpl(aid,$order['mid'],'tmpl_orderfahuo',$tmplcontent,m_url('pages/my/usercenter'));
		//订阅消息
		$tmplcontent = [];
	...
	public function getExpress(){
		...
		$list = \app\commons\Common::getwuliu($order['express_no'],$order['express_com']);
    ...
    function orderHexiao(){
        ...
        Db::name('sales_order')->where('aid',aid)->where('bid',bid)->where('id',$orderid)->update(['status'=>3,'collect_time'=>time()]);
        \app\commons\Member::uplv(aid,$order['mid']);
        \app\commons\System::plog('砍价订单核销确认收货'.$orderid);
	...
	public function refundCheck(){
		...
		$order = Db::name('sales_order')->where('id',$orderid)->where('aid',aid)->where('bid',bid)->find();
		if($st==2){
			Db::name('sales_order')->where('id',$orderid)->where('aid',aid)->where('bid',bid)->update(['refund_status'=>3,'refund_checkremark'=>$remark]);
			...
			\app\commons\Wechat::sendtmpl(aid,$order['mid'],'tmpl_tuierror',$tmplcontent,m_url('pages/my/usercenter'));
			//订阅消息
			$tmplcontent = [];
			...
			Db::name('sales_order')->where('id',$orderid)->where('aid',aid)->where('bid',bid)->update(['status'=>4,'refund_status'=>2]);
			...
			$tmplcontent['orderProductName'] = $order['title'];
			$tmplcontent['orderName'] = $order['ordernum'];
			\app\commons\Wechat::sendtmpl(aid,$order['mid'],'tmpl_tuisuccess',$tmplcontent,m_url('pages/my/usercenter'));
			//订阅消息
			$tmplcontent = [];
	...
	public function del(){
		$id = input('post.id/d');
		Db::name('sales_order')->where('aid',aid)->where('bid',bid)->where('id',$id)->delete();
